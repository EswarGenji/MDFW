...
public class TemplateGenerator {
	...
	private String defaultLogo=GlobalVars.defaultLogo;
	...
	public TemplateGenerator(HashMap<String,List<String>> hMapResultDetails) {
		...
		//this.TestCaseExecutionDetails=TestCaseExecutionDetails;
		//this.testCasesDetails=testCasesDetails;
	...
	public void writeDetailFile(String name,String resultsType, String strChart, String chartDimensions,
		...
		try {
		 ...
		 System.out.println("Totla Time Taken  ::"+time);
		...
		String headContent = "<html> "
        		...
        		+"]);"
        		/*+"var options = {"
        		+"title: '"+Config.release+" Execution Details',"
        		+"pieSliceText: 'label',"
        		+"is3D: true,"
        		+"};"
		...
		String testDetails = "<div id=\"testDetails\">"
				...
				+ "Application : <font color=\"#FFFFFF\">"+ConfigDetails.appName
				+ " </font> " + ln + " </span>" + ln + " </td> " + ln
				+ "<td align=\"left\" valign=\"middle\" class=\"testDetails\">"
				+ ln + "<span  class=\"testDetails\">" + ln + "Test Type :<font color=\"#FFFFFF\"> "
				+  ConfigDetails.release + ln + " </font></span> " + ln + " </td> " + ln
				+ "<td align=\"left\" valign=\"middle\" class=\"testDetails\">"
				+ ln + "<span class=\"testDetails\">" + ln + "Browser: "
				+ConfigDetails.browser + "</span>" + ln + "</td>"
				+ " </tr>" + ln + " </table> " + ln + "</div>";
		...
		String execDetails = "<div id=\"execDetails\"> "
				...
				+"<td align=\"left\" valign=\"middle\" class=\"execDetails\">"
				+ ln + "<span class=\"execDetails\">" + ln + "Total Time : "
				+time+ "</span>" + ln + "</td>"
				+ ln + "</tr>" + ln
				+ "</table>" + ln + "</div> <br/>";
		...
		String buildSummary="<div id=\"buildSummary\" style=\"padding-left:15px\"><span style=\"font-size: 15px;font-weight:bold;color:#000000;font-family:arial;\">Build Summary Details ::</span><p/>"
				+ln
				+"<table id=\"t01\" style=\"width:100%\" border=\"5\"><tr style=\"text-align:center\"><th>Environment</th><th>Application URL's</th><th>DB Server Name</th><th>DB Schema</th><th>Log File</th></tr><tr style=\"text-align:center\"><td>"+ConfigDetails.appName+"</td><td><a href=\""+ConfigDetails.url+"\"  target=\"_blank\">"+ConfigDetails.url+"</td><td>"+ConfigDetails.appName+"</td><td>"+ConfigDetails.appName+"</td><td><a href=\""+scrshotBuild+"\"  target=\"_blank\">Click Here</a></td></tr></table> </div>";

		
		String testCasesExecutionDetails = "<div id=\"testcaseDetails\" style=\"padding-left:15px\">"
				+ ln
				+ " <p> "
				+ "<span style=\"font-size: 15px;font-weight:bold;color:#000000;font-family:arial;\">"+ConfigDetails.release+" Test Cases Execution Details ::</span><p/>"
				+ ln+"<table id=\"t01\" style=\"width:100%\" border=\"5\"><tr  class=\"execution\"><th>SNo</th><th>Module Name</th><th>Test Case ID</th><th>Test Case Title</th><th>Result</th><th>Error Message</th><th>Screenshot</th><th>Time Stamp</th><th>Time Taken (in Seconds)</th></tr>";
		...
		try {
			...
			System.out.println("Hash Map Size ::"+hMapResultDetails.size());
		     for(int row=1;row<=hMapResultDetails.size();row++)
		     {
		    	 ...
		    	 String moduleName=resultDetails.get(1);
		    	 String tcId=resultDetails.get(2);
